        ...
        if middle_output_layers is not None:  
          ...
          # multiple layers distillation
            ...
            if self.uncertainty_loss:
              self.awl = AutomaticWeightedLoss(6)  # codebook_loss1, codebook_loss2 ...
            # if self.momentum:
            ...
            for middle_output_layer, codebook_loss_net, codebook_index in zip(self.middle_output_layers, self.codebook_loss_nets, codebook_indexes):
              ...
              if self.training and codebook_index is not None:
                  ...
                  if codebook_index.shape[-2] != middle_layer_output.shape[-2]:
                    ...
                    sys.exit(1)
                  for i in range(3):
                    codebook_loss = codebook_loss_net(middle_layer_output, codebook_index[i])
                  ...
                  if self.uncertainty_loss:
                    if len(total_losses) == len(self.middle_output_layers) * 3:
